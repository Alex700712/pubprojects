...
public class GetService<T> {
    ...
    public T GetEntityById(RequestType requestType, String id) throws IOException {

        Request findRequest = new Request(requestType, new Gson().toJson(id));
        ClientSocket.getInstance().getOut().println(new Gson().toJson(findRequest));
        ClientSocket.getInstance().getOut().flush();
        String answer;
        answer = ClientSocket.getInstance().getInStream().readLine();
        Response response = new Gson().fromJson(answer, Response.class);
